...
describe('search', () => {
  ...
  it('a VM matches on kind, manager, passthrough and IP', () => {
    const v = { name: 'win', os: 'Windows 10', type: 'virtual-machine', manager: 'truenas', is_vm: true, ipv4: ['192.0.2.141'], passthrough: ['GTX 1050 Ti'] } as VM
    for (const q of ['win', 'windows', 'kvm', 'truenas', 'gtx', '192.0.2']) expect(matchesVM(v, q)).toBe(true)
